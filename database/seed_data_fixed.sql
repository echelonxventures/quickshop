...
-- Insert/update default system configurations using INSERT ... ON DUPLICATE KEY UPDATE
-- This handles the case where configs already exist
...
-- Insert sample categories (these will be duplicates if already existing, so we use ON DUPLICATE KEY UPDATE if needed)
-- but since slug is unique, we'll just insert if they don't exist
INSERT INTO categories (name, slug, description, is_active, sort_order) VALUES
('Electronics', 'electronics', 'Electronic devices and accessories', 1, 1),
('Fashion', 'fashion', 'Clothing, shoes, and accessories', 1, 2),
('Home & Garden', 'home-garden', 'Furniture, home decor, and garden supplies', 1, 3),
